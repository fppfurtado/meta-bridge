...
| `/journal-review [--days N\|--from D --to D] [--interactive] [--write-summary] [--bucket-min-journals N] [--bucket-min-tasks N] [--zombie-days N] [--emerging-min-mentions N] [--cooccur-min-journals N] [--namedrift-max-distance D] [--rename-gap-journals G]` | Skill (thin orchestrator) → `mb journal-review` | Skill aplica 7 heurísticas semânticas (`task-closure-by-context`, `task-zombie`, `bucket-underused`, `bucket-emerging` + trio v2 `bucket-co-occurrence`, `bucket-rename-implicit`, `bucket-naming-drift`) sobre scan estruturado emitido pelo CLI. Preview-first via AskUserQuestion (Aplicar tudo / Cherry-pick / Cancelar). CLI scan emite markers ativos + DONE-tasks + narrativas + inventário de buckets em ordem cronológica + candidatos determinísticos do trio v2 (counting/Levenshtein/gap no CLI); `--apply` aplica transições task-level (heurísticas 1-2) + entries estruturais via payload `## Structural` (A2 archived em page agregadora; B2 emerging no journal de hoje — heurísticas 3-4 saíram de report-only em v0.4.0 per ADR-001 § SD10 Adendo v0.4.0) + sugestões via payload `## Hygiene` (trio v2 → `pages/bucket-hygiene.md`, aditivo forward-only; o CLI gera candidatos, a skill aplica só judgment semântico, per ADR-001 SD10 Adendo 2026-06-24). Skill retém findings em conversation memory entre invocações. Wizard residual via `--interactive`. 7 flags opcionais de threshold (K=2/M=2/T=21/N=4 + cooccur/namedrift/rename=2/2/2 defaults) sobrescrevem heurísticas 2b–2g caso-a-caso. Sucessor de `/weekly-review`. **Bucket `#inbox`**: heurística 1 discrimina entries Forge-synced (finding informacional não-selecionável pra apply) de PKM-native (transição normal) per ADR-001 SD14. |
