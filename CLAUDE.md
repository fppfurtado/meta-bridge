...
- `tests/{test_suggest_session_start_tip.py, test_inbox_aggregate.py, test_logseq_http.py, test_reconcile_check.py}` — suites pytest parciais sob critério parsing-complexo per ADR-002 § Decisão 6 Adendo (2026-06-16); a de `inbox_aggregate` cobre parse + dedup exact-match + write; a de `logseq_http` cobre parse de resposta + config + segurança; a de `reconcile_check` cobre os 3 checks read-only (verify-state + dedup cross-store) + CLI. Demais subcomandos seguem validação manual.
- `docs/decisions/ADR-001-skills-de-bridge.md` — mechanical ADR (18 sub-decisions covering skill internals; SD11 adicionada em v0.9.0 cobrindo `/wiki-compile` per Onda 2; SD12 (2026-06-20) cobrindo `/enrich-blocks` + hook dispatcher; SD13 (2026-06-23) cobrindo `/source-digest` + hook notifier; SD14 (2026-06-23) contrato `#inbox` de reconciliação; SD15 (2026-06-24) `/journal-review` heurística 8 phantom-tag; SD16 (2026-06-25) cobrindo `/wiki-lint` Onda 6 Camada 4; SD17 (2026-06-26) reconciler faceta A — skill `/reconcile` + hook gate-barato + subcomando `reconcile-check` verify-state-at-load read-only; SD18 (2026-06-26) reconciler faceta B — 3º check `cross_store_dedup` dedup cross-store read-only local-first NOTES↔Journal, materializa o componente dedup de meta-system ADR-025).
